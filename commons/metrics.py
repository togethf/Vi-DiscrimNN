...
def bbox_iou(box1, box2):
    ...
    # Intersection area (continuous coordinates, no +1 as in VOC integer pixels)
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                if isinstance(target_labels, torch.Tensor):
                    target_labels_t = target_labels.to(device).long()
                    if not (pred_label.long() == target_labels_t).any():
                        continue
                else:
